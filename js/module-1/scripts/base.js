...
if (login === cancel ) {
    alert (cancelMessage); /* сообщение про отмену юзером в случае нажатии на кнопу отмены в поле логина */
  } else 
    if (login === adminLogin) { 
      const pass = prompt (' Введите пароль:'); /* ввод пароля только если логин совпал */
      if (pass === cancel ) {
        alert (cancelMessage); /* сообщение про отмену юзером в случае нажатии на кнопу отмены в поле пароля */
        } else 
